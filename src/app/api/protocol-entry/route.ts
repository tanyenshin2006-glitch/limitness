...
import { protocolEntryConfirmationEmail } from '@/lib/emailTemplates'
...
export async function POST(req: Request) {
    try {
        ...
        const { data: d1, error: e1 } = await resend.emails.send({
            ...
            to: email,
            subject: 'Protocol Entry — Application Received.',
            html: protocolEntryConfirmationEmail(name)
        })
        ...
        const { data: d2, error: e2 } = await resend.emails.send({
            ...
            subject: 'New Protocol Entry Application',
